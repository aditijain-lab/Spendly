...
Database changes
No database changes. The users and expenses tables already have all required columns (user_id, amount, category, date, description, created_at).

Templates
Modify: templates/profile.html
Amounts must be rendered with the ₹ symbol (Indian Rupee).
All four dynamic sections (user info, summary stats, transaction list, category breakdown) are already present — no structural changes needed, only the Jinja variables they consume are now real.
Files to change
...
pct values in category breakdown must sum to 100; use integer rounding and adjust the largest category to absorb any rounding remainder
If a user has no expenses, summary stats should return zeros and empty lists rather than raising exceptions
Query helpers in database/queries.py must call get_db() internally and close the connection before returning
Tests to write
Unit tests
